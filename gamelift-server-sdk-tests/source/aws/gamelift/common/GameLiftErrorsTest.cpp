...
protected:
    ...
    const std::string ALREADY_INIT_ERROR_MESSAGE = "Server SDK has already been initialized. You must call Destroy() before reinitializing the "
                                                   "client or server.";
    ...
    const char *ALREADY_INIT_ERROR_MESSAGE = "Server SDK has already been initialized. You must call Destroy() before reinitializing the "
                                             "client or server.";
...
TEST_F(GameLiftErrorsTest, GIVEN_statusCodeAndMessage_WHEN_constructError_THEN_success) {
    // GIVEN/WHEN
    GameLiftError error1(400, TEST_ERROR_MESSAGE_1);
