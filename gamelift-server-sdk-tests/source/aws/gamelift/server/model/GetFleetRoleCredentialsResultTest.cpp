...
TEST_F(GetFleetRoleCredentialsResultTest, GIVEN_tooLongStrings_WHEN_SetValues_THEN_cutStringOff) {
    ...
    std::string sessionToken = Utility::TestHelper::GenerateRandomAlphaNumericString(MAX_SESSION_TOKEN_LENGTH);
    ...
    ASSERT_EQ(sessionToken.size(), MAX_SESSION_TOKEN_LENGTH);
    GetFleetRoleCredentialsResult result;
    ...
    ASSERT_EQ(strlen(result.GetSessionToken()), MAX_SESSION_TOKEN_LENGTH);
}
#endif
} // namespace Test
} // namespace Model
