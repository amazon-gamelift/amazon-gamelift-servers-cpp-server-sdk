...
protected:
    ...
    const std::string testSdkToolName = "testSdkToolName";
    const std::string testSdkToolVersion = "testSdkToolVersion";
    ...
    void SetUp() override {
        ...
        testRequest.SetSdkToolName(testSdkToolName);
        testRequest.SetSdkToolVersion(testSdkToolVersion);
        ...
        ss << "{\"Action\":\"" << testAction << "\",\"RequestId\":\"" << testRequestId << "\",\"SdkVersion\":\"" << testSdkVersion << "\",\"SdkLanguage\":\""
           << testSdkLanguage << "\",\"SdkToolName\":\"" << testSdkToolName << "\",\"SdkToolVersion\":\"" << testSdkToolVersion << "\",\"Port\":" << testPort << ",\"LogPaths\":[";
    ...
    void AssertRequestEqualsTestRequest(const ActivateServerProcessRequest &request) {
        ...
        ASSERT_EQ(request.GetSdkToolName(), testSdkToolName);
        ASSERT_EQ(request.GetSdkToolVersion(), testSdkToolVersion);
...
TEST_F(ActivateServerProcessRequestTest, GIVEN_args_WHEN_allArgConstructor_THEN_success) {
    // GIVEN / WHEN
    ActivateServerProcessRequest request(testSdkVersion, testSdkLanguage, testSdkToolName, testSdkToolVersion, testPort, testRequest.GetLogParameters());
    // THEN
    ASSERT_EQ(request.GetAction(), "ActivateServerProcess");
    ASSERT_FALSE(request.GetRequestId().empty());
    ASSERT_EQ(request.GetSdkVersion(), testSdkVersion);
...
TEST_F(ActivateServerProcessRequestTest, GIVEN_validInputs_WHEN_withRequestValues_THEN_success) {
    ...
    Message &messageRef = request.WithSdkVersion(testSdkVersion)
                              ...
                              .WithSdkToolName(testSdkToolName)
                              .WithSdkToolVersion(testSdkToolVersion)
                              .WithPort(testPort)
