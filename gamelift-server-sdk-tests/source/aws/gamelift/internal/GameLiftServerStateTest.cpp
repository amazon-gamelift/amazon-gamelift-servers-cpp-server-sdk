...
#endif
        testing::Mock::AllowLeak(mockWebSocketClientWrapper.get()); // necessary to allow leak of the shared pointer for death testing
...
TEST_F(GameLiftServerStateTest, GIVEN_sdkToolEnvironmentVariable_WHEN_processReady_THEN_sdkToolIsReportedDuringActivateServer) {
    // GIVEN
    EXPECT_CALL(*mockWebSocketClientWrapper, IsConnected()).WillOnce(testing::Return(true));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, testing::AllOf(testing::HasSubstr("\"SdkToolName\":\"testSdkToolName\""), testing::HasSubstr("\"SdkToolVersion\":\"1.0\""))))
        .WillOnce(testing::Return(GenericOutcome(nullptr)));

    // WHEN
    putenv("GAMELIFT_SDK_TOOL_NAME=testSdkToolName");
    putenv("GAMELIFT_SDK_TOOL_VERSION=1.0");

    GenericOutcome outcome = CallProcessReady();

    putenv("GAMELIFT_SDK_TOOL_NAME=");
    putenv("GAMELIFT_SDK_TOOL_VERSION=");

    // THEN
    EXPECT_TRUE(outcome.IsSuccess());
}

TEST_F(GameLiftServerStateTest, GIVEN_noSdkToolEnvironmentVariable_WHEN_processReady_THEN_sdkToolIsNotReportedDuringActivateServer) {
    // GIVEN
    EXPECT_CALL(*mockWebSocketClientWrapper, IsConnected()).WillOnce(testing::Return(true));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, testing::AllOf(testing::Not(testing::HasSubstr("SdkToolName")), testing::Not(testing::HasSubstr("SdkToolVersion")))))
        .WillOnce(testing::Return(GenericOutcome(nullptr)));
...
TEST_F(GameLiftServerStateTest, GIVEN_noHandlerForOnProcessTerminate_WHEN_OnTerminateProcess_THEN_ExitSuccessfully) {
    GTEST_FLAG_SET(death_test_style, "threadsafe");
     // GIVEN
    EXPECT_CALL(*mockWebSocketClientWrapper, IsConnected()).WillRepeatedly(testing::Return(true));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("ActivateServerProcess"))).WillOnce(testing::Return(GenericOutcome(nullptr)));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("HeartbeatServerProcess"))).WillRepeatedly(testing::Return(GenericOutcome(nullptr)));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("TerminateServerProcess"))).WillRepeatedly(testing::Return(GenericOutcome(nullptr)));


    // WHEN-THEN
    GenericOutcome outcome = CallProcessReady();
    long now = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    ASSERT_EXIT(serverState->OnTerminateProcess(now), testing::ExitedWithCode(0), "");
}

TEST_F(GameLiftServerStateTest, GIVEN_noHandlerForOnProcessTerminate_WHEN_OnTerminateProcessFails_THEN_ExitWithFailure) {
    GTEST_FLAG_SET(death_test_style, "threadsafe");
    const GameLiftError expectedError = GameLiftError(400, "test error");
    ...
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("ActivateServerProcess"))).WillOnce(testing::Return(GenericOutcome(nullptr)));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("HeartbeatServerProcess"))).WillRepeatedly(testing::Return(GenericOutcome(nullptr)));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("TerminateServerProcess"))).WillRepeatedly(testing::Return(GenericOutcome(expectedError)));


    // WHEN-THEN
    GenericOutcome outcome = CallProcessReady();
    long now = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    // if static link and windows, then exit will occur with -1
#if !defined(BUILD_SHARED_LIBS) && defined(_WIN32)
    ASSERT_EXIT(serverState->OnTerminateProcess(now), testing::ExitedWithCode(-1), "");
#else
    ASSERT_EXIT(serverState->OnTerminateProcess(now), testing::ExitedWithCode(255), "");
#endif
}
...
TEST_F(GameLiftServerStateTest, GIVEN_processReadyButNoLongerConnected_WHEN_ActivateGameSession_THEN_outcomeFailed) {
    // GIVEN
    EXPECT_CALL(*mockWebSocketClientWrapper, IsConnected())
        .WillOnce(testing::Return(true))   // First call during ProcessReady
        .WillOnce(testing::Return(false)); // Second call during ActivateGameSession, return false to simulate lost connection
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("HeartbeatServerProcess")))
        .WillRepeatedly(testing::Return(GenericOutcome(nullptr)));
    EXPECT_CALL(*mockWebSocketClientWrapper, SendSocketMessage(testing::_, HasAction("ActivateServerProcess")))
        .Times(1)
    ...
    // WHEN
    CallProcessReady();
    const GenericOutcome outcome = serverState->ActivateGameSession();
