...
static const std::regex idempotencyTokenParameterReg("[&?]IdempotencyToken=[0-9a-zA-Z]{32}(?=&|$)");
static const std::regex idempotencyTokenReg("^[0-9a-zA-Z]{32}$");
...
TEST_F(GameLiftServerStateInitTest, GIVEN_envVars_WHEN_InitializeNetworking_THEN_useEnvVars) {
    ...
    // THEN
    std::string uriWithoutIdempotencyToken = regex_replace(capturedUriString, idempotencyTokenParameterReg, "");
    EXPECT_EQ(expectedUri.GetUriString(), uriWithoutIdempotencyToken);
    EXPECT_TRUE(outcome.IsSuccess());

    // Make sure idempotency token is passed
    std::string tokenValue = GetQueryParamValue(capturedUriString, "IdempotencyToken");
    ASSERT_FALSE(tokenValue.empty());
    ASSERT_TRUE(std::regex_match(tokenValue, idempotencyTokenReg));
}
