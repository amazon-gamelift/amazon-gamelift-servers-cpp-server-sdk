...
static const std::regex idempotencyTokenReg("[&?]IdempotencyToken=[0-9a-zA-Z]{32}(?=&|$)");
...
MATCHER_P5(IsCorrectUri, testUrl, testAuthToken, testProcessId, testHostId, testFleetId, "") {
    std::string uriString = arg.GetUriString();
    ...
    std::string uriWithoutIdempotencyToken = regex_replace(uriString, idempotencyTokenReg, "");
    return uri.GetUriString() == uriWithoutIdempotencyToken;
}
...
TEST_F(GameLiftWebSocketClientManagerTest, GIVEN_clientManager_WHEN_connect_THEN_success) {
    ...
    // EXPECT
    EXPECT_CALL(*mockWebSocketClientWrapper, Connect(IsCorrectUri(testUrl + "/", testAuthToken, testProcessId, testHostId, testFleetId)))
...
TEST_F(GameLiftWebSocketClientManagerTest, GIVEN_clientManager_WHEN_connectForbidden_THEN_fail) {
    ...
    // EXPECT
    EXPECT_CALL(*mockWebSocketClientWrapper, Connect(IsCorrectUri(testUrl + "/", testAuthToken, testProcessId, testHostId, testFleetId)))
        .WillOnce(testing::Return(GenericOutcome(GAMELIFT_ERROR_TYPE::WEBSOCKET_CONNECT_FAILURE_FORBIDDEN)));
    // WHEN
    GenericOutcome outcome = clientManager->Connect(testUrl, testAuthToken, testProcessId, testHostId, testFleetId);
    // THEN
    ASSERT_FALSE(outcome.IsSuccess());
...
TEST_F(GameLiftWebSocketClientManagerTest, GIVEN_clientManager_WHEN_connectTimeout_THEN_fail) {
    ...
    // EXPECT
    EXPECT_CALL(*mockWebSocketClientWrapper, Connect(IsCorrectUri(testUrl + "/", testAuthToken, testProcessId, testHostId, testFleetId)))
        .WillOnce(testing::Return(GenericOutcome(GAMELIFT_ERROR_TYPE::WEBSOCKET_CONNECT_FAILURE_TIMEOUT)));
    // WHEN
    GenericOutcome outcome = clientManager->Connect(testUrl, testAuthToken, testProcessId, testHostId, testFleetId);
    // THEN
    ASSERT_FALSE(outcome.IsSuccess());
